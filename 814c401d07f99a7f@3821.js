...
md`# (New Mousetrap) Supply and Demand chart`
)}
...
[
  {
    ...
    verticalLabelFormat: ".0f", // format according to 'd3-format' spec https://github.com/d3/d3-format
    
    horizontalLabelOffset: [0, 0],
    horizontalLabelColor: 'black',
    horizontalLabelFormat: ".0f",
  }
]
)}
...
function _interactionBoundary(xAxisScale,yAxisScale){return(
{xMin: xAxisScale.domain[0], xMax: xAxisScale.domain[1], yMin: yAxisScale.domain[0], yMax: yAxisScale.domain[1]}
)}

function _cutLineOnBoundaryCross(){return(
true
)}
...
function _lineMap(){return(
{}
)}

function _drag(svg,xScale,yScale,getColor,lineMap,d3,appendIntersectionObjects,appendLabels,interactionBoundary,yAxisScale,xAxisScale,intersectLines,yAxisLine,xAxisLine,topLine,rightLine,distance,xyScale,LINE_MIN_LENGTH,GHOST_MIN_DISTANCE,getCursor)
{
  function dragstarted(event, d) {
    ...
    if (d.drawOriginLine && !d.hasOwnProperty('isAwayFromOrigin')) {
      ...
      lineMap[`line.ghost_${d.id}_x1`] = xScale(d.originalP1[0]);
      lineMap[`line.ghost_${d.id}_y1`] = yScale(d.originalP1[1]);
      lineMap[`line.ghost_${d.id}_x2`] = xScale(d.originalP2[0]);
      lineMap[`line.ghost_${d.id}_y2`] = yScale(d.originalP2[1]);

      d.isAwayFromOrigin = true;
    }
    // following times, add color back 
    else if (d.drawOriginLine && !d.isAwayFromOrigin) {
      svg.select(`line.ghost_${d.id}`)
        .attr('opacity', '1')
      
      d3.select(`text.ghost_${d.id}`)
        .attr('opacity', '1')
  ...
  function dragged(event, d) {
    ...
    // branch for horizontal lines
    if (d.point1[1] == d.point2[1]) {
      if (d.point1[1] < interactionBoundary.yMin) {
        d.point1[1] += (yAxisScale.domain[1] - yAxisScale.domain[0]) / 100;
        d.point2[1] += (yAxisScale.domain[1] - yAxisScale.domain[0]) / 100;
      }
      else if (d.point1[1] > interactionBoundary.yMax) {
        d.point1[1] -= (yAxisScale.domain[1] - yAxisScale.domain[0]) / 100;
        d.point2[1] -= (yAxisScale.domain[1] - yAxisScale.domain[0]) / 100;
      }
    }
    // vertical lines
    else if (d.point1[0] == d.point2[0]) {
      if (d.point1[0] < interactionBoundary.xMin) {
        d.point1[0] += (xAxisScale.domain[1] - xAxisScale.domain[0]) / 100;
        d.point2[0] += (xAxisScale.domain[1] - xAxisScale.domain[0]) / 100;
      }
      else if (d.point1[0] > interactionBoundary.xMax) {
        d.point1[0] -= (xAxisScale.domain[1] - xAxisScale.domain[0]) / 100;
        d.point2[0] -= (xAxisScale.domain[1] - xAxisScale.domain[0]) / 100;
      }
    }
    // branch for all other lines
    else {
  ...
  function dragended(event, d) {
    ...
    if (d.drawOriginLine) {
      const ghostLineX1 = lineMap[`line.ghost_${d.id}_x1`]
      const ghostLineY1 = lineMap[`line.ghost_${d.id}_y1`]
      const ghostLineX2 = lineMap[`line.ghost_${d.id}_x2`]
      const ghostLineY2 = lineMap[`line.ghost_${d.id}_y2`]
...
function _appendStaticIntersectionObjects(intersectionPoints,intersectLines,getLineByLabel,xAxisScale,yAxisScale,svg,dropLines,xScale,height,margin,yScale){return(
...
function _appendIntersectionObjects(d3,svg,textShadow,intersectionPoints,intersectLines,getLineByLabel,xAxisScale,yAxisScale,dropLines,xScale,height,margin,yScale){return(
  ...
  intersectionPoints.map( pair => {
        ...
        point.y >= yAxisScale.domain[0] && point.y <= yAxisScale.domain[1]) 
    {
      ...
      svg.append('text')
        ...
        .style("font-size", 18)
        .style("font-weight", 'bold')
        .text(() => `${d3.format(pair.verticalLabelFormat)(point.x)}`)
      ...
      svg.append('text')
        ...
        .style("font-size", 18)
        .style("font-weight", 'bold')
        .text(() => `${d3.format(pair.horizontalLabelFormat)(point.y)}`)
...
function _yAxisLine(interactionBoundary){return(
{
...
function _xAxisLine(interactionBoundary){return(
{
  point1: [interactionBoundary.xMin, interactionBoundary.yMin],
...
function _topLine(interactionBoundary){return(
{
...
function _rightLine(interactionBoundary){return(
{
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("interactionBoundary")).define("interactionBoundary", ["xAxisScale","yAxisScale"], _interactionBoundary);
  ...
  main.variable(observer("drag")).define("drag", ["svg","xScale","yScale","getColor","lineMap","d3","appendIntersectionObjects","appendLabels","interactionBoundary","yAxisScale","xAxisScale","intersectLines","yAxisLine","xAxisLine","topLine","rightLine","distance","xyScale","LINE_MIN_LENGTH","GHOST_MIN_DISTANCE","getCursor"], _drag);
